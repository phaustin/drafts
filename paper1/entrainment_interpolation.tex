...
report found a *TK* W m$^{-2}$ range in the cloud feedback response, relative 
to a *TK* W m$^{-2}$ total climate sensitivity.  Improvements in the accuracy 
of these subgridscale cloud parametrizations are neccessary to simulate the 
spatial pattern and rate of global warming.
...
taken from field surveys such as the Barbados Oceanographic and Meteorological
...
$\Delta x$ is the model grid spacing and $\Delta t$ is the model timestep.  The
...
of 6.4 km x 6.4 km in the horizontal and 3.2 km in the vertical.  Calculations 
of cloud area, vertical velocity, and entrainment diagnosed from bulk conserved
tracer budgets agree well with the results presented in \cite{Siebesma2003}.

To examine resolution dependence of our scheme, we ran three models: one at 25m 
grid spacing in all directions with a 1.5 second timestep, one at 50m grid 
spacing with a 3 second timestep, and one at 100m grid spacing with a 6 second 
...
/cite{Romps} very recently created a complimentary technique to measure 
...
time needed for an entire grid cell to entrain or detrain.

* values are smaller than ours
* values are likely more accurate--fewer assumptions
* nevertheless, scaling E/D for the specific question being considered is still
  neccesary.
* does not result in instantaneous values; our system is more appropriate for
  cloud tracking
