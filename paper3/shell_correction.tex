...
budget calculations.  Here we explain this difference as resulting from a bias 
in the bulk tracer calculations due to the presence of the moist, negatively 
buoyant shell of evaporated cloud air that surrounds a cloud.

%==============================================================================

\section{Correction of conserved bulk tracer calculations}
...
\subsection{Correcting Mass Flux Entrainment for Tracer Entrainment}
...
\cite{Siebesma1995}, replacing the mean cloud and environment values of $\chi$ 
in the entrainment and detrainment terms with the cloud and environment values 
at the cloud surface, $\chi_{se}$ and $\chi_{sc}$:
\begin{eqnarray}
...
the uncorrected value of the entrainment by a factor 
...
We can easily invert equations (\ref{eq:final_entrainment}) and 
(\ref{eq:final_detrainment}) to use the direct values of \cite{Romps2010} to 
calculate the equivalent tracer entrainment values:
\begin{equation}
...
However, \cite{Romps2010} derives alternate equations equivalent to 
(\ref{eq:siebesma_entrainment}) and (\ref{eq:siebesma_detrainment}) which use
direct mass flux calculations in place of horizontally averaged tracer budgets.
\cite{Romps2010} derives tracer entrainment and detrainment to be:
\begin{equation}
  \label{eq:romps_entrainment}
  E_0 = \frac{\chi_{c}(E-D) - (\overline{e\chi} - \overline{d\chi})}{(\chi_{c} - \chi_{e})}
\end{equation}
\begin{equation}
  \label{eq:romps_detrainment}
  D_0 = \frac{\chi_{e}(E-D) - (\overline{e\chi} - \overline{d\chi})}{(\chi_{c} - \chi_{e})}
\end{equation}
...
\begin{figure}
  ...
  \noindent\includegraphics[width=20pc]{./figures/resolution_dependence}
  \caption{Mean profiles of a) Entrainment and b) Detrainment.}
\end{figure}

\begin{figure}
  \label{fig:shell_edge_profiles}
  \noindent\includegraphics[width=29pc]{./figures/shell_edge_profiles_core}
...
\begin{figure}
  ...
  \noindent\includegraphics[width=39pc]{./figures/corrected_entrainment_core}
